...
def auto_save_metric_plots(df, time_step, rolling_over,lr,opt):
    ...
    labels=['{:.4E}_{}'.format(float(l),o) for l,o in zip(lr,opt)]
    ...
    plot_labels=['num_idle','longwait_pass','num_serving','total_reward','removed_pass']
    fig, axes = plt.subplots(nrows=len(plot_labels), ncols=2, figsize=(10, 6*len(plot_labels)))
    axe = axes.ravel()


    for d,label in zip(df,labels):
        ax_id = 0
        for id, col in enumerate(d.columns.tolist()[1:]):
            ...
            print(len(v))
            axe[ax_id].plot(v[1:-1],'-.*', label=label,lw=0.5,ms=2)
...
if __name__ == "__main__":
    ...
    learning_rate=['0.00014933','0.00014933']
    options=[0,1,1,1,1]
    ...
    # df+=[pd.read_csv('../logs/parsed_results_{}_{}_nc.csv'.format(i,lr)) for i in options]
    # #
    auto_save_metric_plots(df, 1, 1440,learning_rate,options)