...
from config.hex_setting import NUM_REACHABLE_HEX, NUM_NEAREST_CS, ENTERING_TIME_BUFFER, HEX_ROUTE_FILE, \
    ...
    N_DUMMY_VEHICLES, N_DQN_VEHICLES
...
class Simulator(object):
    ...
    def init(self, file_hex, file_charging, trip_file, travel_time_file, n_nearest=NUM_NEAREST_CS):
        ...
        xy_coords = df[['row_id','col_id']].to_numpy()
        ...
        for h_idx, coords, xy_coord, match_id in zip(hex_ids, hex_coords, xy_coords, hex_to_match):
            ...
            self.hex_zone_collection[h_idx] = hex_zone(h_idx, coords, xy_coord, hex_coords, match_id, neighbors, charging_idx,
    ...
    def get_states(self):
        ...
        dqn_agents = [vehicle for hex in self.hex_zone_collection.values() for vehicle in hex.vehicles.values() if
                      ...
                      vehicle.state.status in [status_codes.V_IDLE,status_codes.V_STAY]]
        ...
        if len(dqn_agents) <= 0:
            ...
            # non_valid_cs_ids = [int(len(cs.queue)>=QUEUE_LENGTH_THRESHOLD) for cs in self.charging_station_collections]
