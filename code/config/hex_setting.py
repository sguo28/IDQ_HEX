...
GAMMA = 1 - 1e-5
REPLAY_BUFFER_SIZE = int(6e6)
BATCH_SIZE = 256
FINAL_EPSILON = 0.05
START_EPSILON = 1.0
EPSILON_DECAY_STEPS = 4.5e5
CLIPPING_VALUE = 1.0
RELOCATION_DIM = 7
CHARGING_DIM = 5
INPUT_DIM = 2 + 11 + NUM_REACHABLE_HEX
...
TIMESTEP = 60  # sec
ENTERING_TIME_BUFFER = 60 * 60  # 60*60*1 = 1 hour

REJECT_TIME = 30 * 60  # 30 min
START_OFFSET = int(0)  # simulation start datetime offset (days)"
SIM_DAYS = int(350)  # simulation days: 504000 ticks
...
### Other config for simulator

N_VEHICLES = int(7000)
N_DUMMY_VEHICLES = int(0)
N_DQN_VEHICLES = int(7000)
ALL_SUPERCHARGING = True  # mask level 2 type to super-charging.
QUEUE_LENGTH_THRESHOLD = 5  # 5 * 40 min if DC-fast
QUICK_END_CHARGE_PENALTY = 300  # equivalent to weighted summation of 200 min staying at charging station (0.5*200min) + charge cost (5*$12)
