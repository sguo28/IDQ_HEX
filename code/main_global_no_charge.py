...
from logs.parse_results_cnn import save_global_metric_plots
...
if __name__ == '__main__':
    if SIM_DAYS > 0:
        ...
        dqn_agent = DeepQNetworkAgent(NO_CHARGE, GLOBAL_MATCHING)
        # engine = OSRMEngine()
        n_steps = int(3600 * 24 / TIMESTEP)  # 60 per minute
        with open('logs/parsed_results_cnn_global.csv', 'w') as f, open(
                'logs/target_charging_stations_cnn_global.csv', 'w') as g, \
                open('logs/training_hist_cnn_global.csv', 'w') as h, open(
            'logs/demand_supply_gap_cnn_global.csv', 'w') as l1, \
                open('logs/cruising_od_cnn_global.csv', 'w') as m1, open(
            'logs/matching_od_cnn_global.csv', 'w') as n1:
            if not CNN_RESUME:  # if not continueing training: set up column names:
                f.writelines(
                ...
                h.writelines('{},{},{},{},{},{}\n'.format("step", "loss", "reward", "learning_rate",
                                                          "sample_reward", "sample_SOC"))
            ...
            for day in range(SIM_DAYS):
                ...
                for i in range(n_steps):
                    ...
                    # local_state_batches, num_valid_relos, od_pair_lists = simulator.get_local_infos()
                    ...
                    t_query = time.time()
                    # print('od list',od_pair_lists)
                    # response = engine.route_hex(od_pair_lists, decode=False, annotations=False)
                    # simulator.attach_route_info(response)
                    ...
                    if tick % STORE_TRANSITION_CYCLE == 0:
                        ...
                        if states is not None:
                            [dqn_agent.add_transition(state, action, next_state, reward, valid_action_num_) for
                             state, action, next_state, reward, valid_action_num_ in
                             zip(states, actions, next_states, rewards, valid_action_nums_)]
                    ...
                    if tick % (60 * 60 * 24 * 5) == 0 and tick != 0:  # plot per 5 days while rolling over on per day.
                        metric_df = pd.read_csv('logs/parsed_results_cnn_global.csv')
                        train_df = pd.read_csv('logs/training_hist_cnn_global.csv')
                        save_global_metric_plots(metric_df, train_df, tick // (60 * 60 * 24 * 5))
